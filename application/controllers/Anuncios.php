...
class Anuncios extends CI_Controller {
    ...
    public function home($pag = null) {
        ...
        if($pag == null || $pag == "home" || $pag == "aguardando"){
            $this->session->set_userdata('pag_anuncio', 'aguardando');
            $dados['anuncios'] = $this->anuncios_model->anuncios_aguardando();
            $this->load->view('anuncios/home', $dados);
        }
        if($pag == "ativos"){
            $this->session->set_userdata('pag_anuncio', 'ativos');
            $dados['anuncios'] = $this->anuncios_model->anuncios_ativos();
            $this->load->view('anuncios/home', $dados);
        }
        if($pag == "concluidos"){
            $this->session->set_userdata('pag_anuncio', 'concluidos');
            $dados['anuncios'] = $this->anuncios_model->anuncios_concluidos();
            $this->load->view('anuncios/home', $dados);
        }
        if($pag == "nao_concluidos"){
            $this->session->set_userdata('pag_anuncio', 'nao_concluidos');
            $dados['anuncios'] = $this->anuncios_model->anuncios_nao_concluidos();
            $this->load->view('anuncios/home', $dados);
        }
    ...
    public function criar(){
        $this->crud->inicia();
        $this->crud->add_campo('com_usuario', 'nome', 'oculto', 1, $this->session->userdata['usu_id']);
        $this->crud->add_campo('com_categoria', 'Categoria:', 'select', 1, $this->crud->listar_combo('categoria', 'cat_id', 'cat_nome'));
        $this->crud->add_campo('com_nome', 'Nome:', 'text', 1);
        $this->crud->add_campo('com_descricao', 'Descrição:', 'textarea', 1);
        $this->crud->add_campo('com_preco', 'Preco que deseja pagar:', 'text', 1);
        $this->crud->add_campo('com_status', 'status', 'oculto', 1, '0');
        $this->crud->add_campo('enviar', 'Inserir', 'submeter', 0);
        $this->crud->inserir('enviar', 'compra', $this->crud->fim('info'));
        ...
        $this->load->view('header');
        $this->load->view('barra_superior', $dados);
        $this->load->view('anuncios/criar', $dados);
        $this->load->view('footer');
    }
