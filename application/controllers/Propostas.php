...
class Propostas extends CI_Controller {
    ...
    public function index(){
        redirect('propostas/home');
    }


    public function home($pag = null) {
        $dados['user'] = $this->painel_model->retorna_dados_usuario();
        $this->load->view('header');
        $this->load->view('barra_superior', $dados);

        if($pag == null || $pag == "home" || $pag == "feitas"){
            $this->session->set_userdata('pag_propostas', 'feitas');
            $dados['list'] = $this->crud->listar('propostas', 'pro_id', 'pro_descricao', 'propostas', array('pro_usuario' => $this->session->userdata['usu_id'], 'pro_status' => 0));
            $this->load->view('propostas/home', $dados);
        }
        if($pag == "aceitas"){
            $this->session->set_userdata('pag_propostas', 'aceitas');
            $dados['list'] = $this->crud->listar('propostas', 'pro_id', 'pro_descricao', 'propostas', array('pro_usuario' => $this->session->userdata['usu_id'], 'pro_status' => 1));
            $this->load->view('propostas/home', $dados);
        }
        if($pag == "recusadas"){
            $this->session->set_userdata('pag_propostas', 'recusadas');
            $dados['list'] = $this->crud->listar('propostas', 'pro_id', 'pro_descricao', 'propostas', array('pro_usuario' => $this->session->userdata['usu_id'], 'pro_status' => 2));
            $this->load->view('propostas/home', $dados);
        }
    ...
    public function criar($id){
        $this->crud->inicia();
        $this->crud->add_campo('pro_compra', 'anuncio', 'oculto', 1, $id);
        $this->crud->add_campo('pro_status', 'status', 'oculto', 1, 0);
        $this->crud->add_campo('pro_usuario', 'nome', 'oculto', 1, $this->session->userdata['usu_id']);
        $this->crud->add_campo('pro_preco', 'Preco que voce esta pedindo:', 'text', 1);
        $this->crud->add_campo('pro_descricao', 'Descrição:', 'textarea', 0);
        $this->crud->add_campo('enviar', 'Inserir', 'submeter', 0);
        $this->crud->inserir('enviar', 'propostas', $this->crud->fim('info'));

        $dados['formulario'] = $this->crud->fim('form');
        $dados['user'] = $this->painel_model->retorna_dados_usuario();

        $this->load->view('header');
        $this->load->view('barra_superior', $dados);
        $this->load->view('propostas/criar', $dados);
        $this->load->view('footer');
    }

    public function alterar($id){
        $dados['user'] = $this->painel_model->retorna_dados_usuario();
        $this->session->set_userdata('avisos', 'danger|Esta funcionalidade não está habilitada!');
        $this->load->view('header');
        $this->load->view('barra_superior', $dados);
        $this->load->view('footer');
        
        /*$res = $this->crud->consultar('compra', 'com_id', $id);
        $this->crud->inicia();
        $this->crud->add_campo('com_categoria', 'Categoria:', 'select', 1, $this->crud->listar_combo('categoria', 'cat_id', 'cat_nome'), $res->com_categoria);
        $this->crud->add_campo('com_nome', 'Nome:', 'text', 1, $res->com_nome);
        $this->crud->add_campo('com_descricao', 'Descrição:', 'textarea', 1, $res->com_descricao);
        $this->crud->add_campo('com_preco', 'Preco que deseja pagar:', 'text', 1, $res->com_preco);
        $this->crud->add_campo('enviar', 'Alterar', 'submeter', 0);
        $this->crud->alterar('enviar', 'compra', $this->crud->fim('info'), 'com_id', $id);

        $dados['formulario'] = $this->crud->fim('form');
        $dados['user'] = $this->painel_model->retorna_dados_usuario();

        //var_dump($this->crud->listar_combo('categoria', 'cat_id', 'cat_nome'));
        $this->load->view('header');
        $this->load->view('barra_superior', $dados);
        $this->load->view('anuncios/alterar', $dados);
        $this->load->view('footer');*/
    }


    public function apagar($id){
        $this->crud->confirma_apagar('propostas', array('pro_id' => $id));
        $dados['user'] = $this->painel_model->retorna_dados_usuario();
        $dados['pre_footer'] = $this->crud->apagar();
        $this->load->view('header');
        $this->load->view('barra_superior', $dados);
