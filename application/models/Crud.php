...
class Crud extends CI_Model {
    ...
    function inicia(){
        ...
        global $list;
        $info = array();
        $form = $form . "<form method=\"post\" action=\"\" enctype=\"multipart/form-data\">";
    }
    ...
    function fim($tipo){
        ...
        //return array('form' => $form, 'info' => $info);
    }

    function listar($table, $campo_id, $campo_nome, $pag, $where = null, $inner = null, $mais_campos = null){
        global $list;
        $list = $list . "<table class=\"table table-striped table-bordered table-hover\" id=\"dataTables-example\">
            <thead> 
                <tr> 
                    <th>#</th> 
                    <th>$campo_nome</th>";
                    /*if($mais_campos != null){
                        foreach ($mais_campos as $n) {
                            $list = $list . "<th>$n</th>";
                        }
                    }*/
                    $list = $list . "
        ...
        foreach ($result as $n) {
            $cont++;
            $n->id = $n->$campo_id;
            $n->valor = $n->$campo_nome;
            $list = $list . "
                <tr>
                    <td>$cont</td>

                    <td><a href=\"".base_url().$pag."/home/ver/" . $n->$campo_id . "\">" . $n->$campo_nome. "</a></td>
                    
                    <td><a href=\"".base_url().$pag."/alterar/".$n->$campo_id."\" class=\"btn btn-outline btn-warning\">Atualizar</a></td>
                    
                    <td><a href=\"".base_url().$pag."/apagar/".$n->$campo_id."\" class=\"btn btn-outline btn-danger\">Apagar</a></td>
    ...
    function apagar(){
        ...
        <div class=\"row\">
            <div class=\"col-lg-12\">
                <h1 class=\"page-header\">Apagar</h1>
                <h3><b>Tem certeza que deseja apagar o registro?</b></h3>
                <br>
    ...
    function add_campo($nome, $label, $tipo, $obrigatorio, $valor = null, $selecionado = null, $max = null){
        global $form;
        global $info;
        $inf = array(
            'nome' => $nome,
        ...
        if($tipo == "text"){
            ...
            <div class=\"form-group\">
                <label>$label</label>
                <input type=\"text\" name=\"$nome\" value=\"$valor\" maxlength=\"$max\" class=\"form-control\">
            </div>";
        }
        if($tipo == "textarea"){
            $form = $form . "
            <div class=\"form-group\">
                <label>$label</label>
                <textarea name=\"$nome\" maxlength=\"$max\" rows=\"10\" class=\"form-control\">$valor</textarea>
            </div>";
        }
        if($tipo == "select"){
            if($selecionado == null){
                ...
                <div class=\"form-group\">
                    ...
                    <select name=\"$nome\" class=\"form-control\">
                        <option>Selecione um(a) $label</option>";
                        foreach ($valor as $n) {
                            $form = $form . "<option value=\"" . $n->id . "\">" . $n->valor . "</option>";
                        }
                        $form = $form . "
                    </select> 
                </div>";
                ...
                <div class=\"form-group\">
                    ...
                    <select name=\"$nome\" class=\"form-control\">
                        ...
                        foreach ($valor as $n) {
                            if($n->id == $selecionado){
                                $form = $form . "<option value=\"" . $n->id . "\" selected>" . $n->valor . "</option>";
                            }else{
                                $form = $form . "<option value=\"" . $n->id . "\">" . $n->valor . "</option>";
                            }
                        }
        ...
        if($tipo == "submeter"){
            ...
            <div style=\"float: right;\">
                <input type=\"submit\" name=\"$nome\" value=\"$label\" class=\"btn btn-outline btn-success\">
            </div>";
        }
    }


    function inserir($submissao, $tabela, $info){
        if($this->input->post($submissao)){
            $data = array();
            //unset($info[''])
            foreach ($info as $n) {
                if($n['tipo'] != 'submeter'){
                    if($n['tipo'] != 'oculto'){
                        $campo = $this->input->post($n['nome']);
                        if($n['obrigatorio'] == 1){
                            if(!empty($campo)){
                                $data[$n['nome']] = $campo;
                            }else{
                                $this->session->set_userdata('avisos', 'danger|Por favor! Preencha todos os campos obrigatórios!');
                            }
                        }else{
                            $data[$n['nome']] = $campo;
                        }
            ...
            if($this->db->affected_rows()){
                $this->session->set_userdata('avisos', 'success|Operação realizada com sucesso!');
            }else{
                $this->session->set_userdata('avisos', 'danger|Houve um problema! Por favor contate o administrador!');
            }
        }
    }

    function alterar($submissao, $tabela, $info, $campo_id, $id){
        if($this->input->post($submissao)){
            $data = array();
            //unset($info[''])
            foreach ($info as $n) {
    ...
    function confirma_apagar($tabela, $where){
    ...
    function listar_combo($tabela, $id, $valor){
        $this->db->select('*');
        $this->db->from($tabela);
        $result = $this->db->get()->result();
        foreach ($result as $n) {
   ...
   /* function anuncios_aguardando(){
        $this->db->select('*');
        $this->db->from('compra');
        $this->db->where('com_status', 0);
        $this->db->where('com_usuario', $this->session->userdata['usu_id']);
        return $this->db->get()->result();
    }
    function anuncios_ativos(){
        $this->db->select('*');
        $this->db->from('compra_ativo');
        $this->db->where('com_usuario', $this->session->userdata['usu_id']);
        return $this->db->get()->result();
    }
    function anuncios_concluidos(){
        $this->db->select('*');
        $this->db->from('compra');
        $this->db->where('com_status', 2);
        $this->db->where('com_usuario', $this->session->userdata['usu_id']);
        return $this->db->get()->result();
    }
    function anuncios_nao_concluidos(){
        $this->db->select('*');
        $this->db->from('compra');
        $this->db->where('com_status', 3);
        $this->db->where('com_usuario', $this->session->userdata['usu_id']);
        return $this->db->get()->result();
    }
    ...
    function qtde_anuncios_ativos(){
        $this->db->select('count(*) as qtde');
        $this->db->from('compra_ativo');
        $this->db->where('com_usuario', $this->session->userdata['usu_id']);
        return $this->db->get()->result()[0];
    }
    function qtde_anuncios_concluidos(){
        $this->db->select('count(*) as qtde');
        $this->db->from('compra');
        $this->db->where('com_status', 2);
        $this->db->where('com_usuario', $this->session->userdata['usu_id']);
    ...
    function qtde_anuncios_nao_concluidos(){
        $this->db->select('count(*) as qtde');
        $this->db->from('compra');
        $this->db->where('com_status', 3);
        $this->db->where('com_usuario', $this->session->userdata['usu_id']);
